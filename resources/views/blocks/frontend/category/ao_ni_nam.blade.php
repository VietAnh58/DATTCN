                    ...
                    <figure>
                        <a href="{{ route('detail_product', $item->alias) }}">
                            <img src="{{ asset('storage/images') }}/{{ $item->image }}" width="280" height="280"
                                alt="product" />
                            {{-- <img src="{{ asset('frontend/')}}/assets/images/ao_ni_nam/product-2-2.jpg" width="280" height="280" alt="product" /> --}}
...
@section('page')
    <!-- Nút trang trước -->
    @if ($ao_ni_nam->currentPage() == 1)
        <li class="page-item disabled">
            <a class="page-link page-link-btn" href="#"><i class="fa-solid fa-angle-left"></i></i></a>
        </li>
    @else
        <li class="page-item">
            <a class="page-link page-link-btn" href="{{ $ao_ni_nam->previousPageUrl() }}"><i
                    class="fa-solid fa-angle-left"></i></i></a>
        </li>
    @endif

    <!-- Các trang được liệt kê -->
    @for ($i = 1; $i <= $ao_ni_nam->lastPage(); $i++)
        <li class="page-item {{ $i == $ao_ni_nam->currentPage() ? 'active' : '' }}">
            <a class="page-link" href="{{ $ao_ni_nam->url($i) }}">{{ $i }}</a>
        </li>
    @endfor

    <!-- Nút trang tiếp theo -->
    @if ($ao_ni_nam->currentPage() == $ao_ni_nam->lastPage())
        <li class="page-item disabled">
            <a class="page-link page-link-btn" href="#"><i class="fa-solid fa-angle-right"></i></i></a>
        </li>
    @else
        <li class="page-item">
            <a class="page-link page-link-btn" href="{{ $ao_ni_nam->nextPageUrl() }}"><i
