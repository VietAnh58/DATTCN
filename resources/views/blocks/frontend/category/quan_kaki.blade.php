                    ...
                    <figure>
                        <a href="{{ route('detail_product', $item->alias) }}">
                            <img src="{{ asset('storage/images') }}/{{ $item->image }}" width="280" height="280"
                                alt="product" />
                            {{-- <img src="{{ asset('frontend/')}}/assets/images/quan_kaki/product-2-2.jpg" width="280" height="280" alt="product" /> --}}
...
@section('page')
    <!-- Nút trang trước -->
    @if ($quan_kaki->currentPage() == 1)
        <li class="page-item disabled">
            <a class="page-link page-link-btn" href="#"><i class="fa-solid fa-angle-left"></i></i></a>
        </li>
    @else
        <li class="page-item">
            <a class="page-link page-link-btn" href="{{ $quan_kaki->previousPageUrl() }}"><i
                    class="fa-solid fa-angle-left"></i></i></a>
        </li>
    @endif

    <!-- Các trang được liệt kê -->
    @for ($i = 1; $i <= $quan_kaki->lastPage(); $i++)
        <li class="page-item {{ $i == $quan_kaki->currentPage() ? 'active' : '' }}">
            <a class="page-link" href="{{ $quan_kaki->url($i) }}">{{ $i }}</a>
        </li>
    @endfor

    <!-- Nút trang tiếp theo -->
    @if ($quan_kaki->currentPage() == $quan_kaki->lastPage())
        <li class="page-item disabled">
            <a class="page-link page-link-btn" href="#"><i class="fa-solid fa-angle-right"></i></i></a>
        </li>
    @else
        <li class="page-item">
            <a class="page-link page-link-btn" href="{{ $quan_kaki->nextPageUrl() }}"><i
